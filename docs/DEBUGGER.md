...
> **`vs_wait_chains` and `vs_async_stacks` run ClrMD out-of-process.** ClrMD can't load in-proc in devenv (its `System.Collections.Immutable` bind collides with devenv's own binding policy → `MissingMethodException`), so a bundled worker (`ClrMdWorker.exe`, with its own binding config) takes the snapshot in a separate process and returns JSON. That snapshot is a `PssCaptureSnapshot` **fork**, so it reads a clone and coexists with the live VS debug session. Both need an active session (a Break All first gives the cleanest read), are managed-only, and take an optional `pid` for multi-process sessions.
