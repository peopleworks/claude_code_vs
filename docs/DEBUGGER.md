...
| `vs_wait_chains` | **Structured deadlock triage** via a ClrMD process snapshot: every HELD monitor with its owner thread + waiter count, each thread's held locks + blocked state, and `deadlockSuspects` (threads that hold a lock *and* are blocked entering a monitor = the cycle members). Exact ownership, not text-derived. Pair with `vs_threads` for the explicit waiting-on edge. |
| `vs_async_stacks` | **Logical async call-stack reconstruction** via a ClrMD snapshot: walks the heap's async state-machine boxes and returns each in-flight async chain (innermost first) with its await-point `state` — the `RunAsync → ComputeAsync → InnerAsync` chain the physical stack hides. |
| `vs_heap_stats` | **Memory snapshot**: top managed types by total bytes (count + size), bytes per GC generation (gen0/1/2/LOH/POH), GC mode (server/workstation, regions, background), GC-handle counts by kind, finalizer-queue size + top finalizable types. The "what's using memory / what looks off" overview. |
| `vs_threadpool` | **ThreadPool health**: worker counts (min/max/existing/busy/goal), queued work-item backlog, and a `starved` flag — the "async app hangs but nothing's deadlocked" diagnosis (pool threads blocked, work piling up). Pair with `vs_async_stacks`. .NET 6+ targets. |
| `vs_gc_roots` | **"Why is this alive?"**: give a type name or `0x`-address → the retention path from a GC root to an instance (each frame references the next), with `rootKind` (static / stack local / handle / finalizer). The leak root-cause tool. |
| `vs_heap_diff` | **Leak finder**: first call baselines the heap; later calls report what GREW (per-type count/byte deltas). A type climbing across repeated calls is the leak — then `vs_gc_roots` it. `reset` re-baselines. |

> **The six ClrMD tools** (`vs_wait_chains`, `vs_async_stacks`, `vs_heap_stats`, `vs_threadpool`, `vs_gc_roots`, `vs_heap_diff`) **run ClrMD out-of-process.** ClrMD can't load in-proc in devenv (its `System.Collections.Immutable` bind collides with devenv's own binding policy → `MissingMethodException`), so a bundled worker (`ClrMdWorker.exe`, with its own binding config) takes the snapshot in a separate process and returns JSON. That snapshot is a `PssCaptureSnapshot` **fork**, so it reads a clone and coexists with the live VS debug session. All need an active session, are managed-only, and take an optional `pid` for multi-process sessions.
...
- **Lock ownership: structured for monitors, partial beyond.** `vs_wait_chains` (ClrMD) now gives **exact** monitor ownership — every held monitor's holder + waiter count, plus the `deadlockSuspects` cycle members — and `vs_threads` adds the explicit `lockOwnerThreadId` edge (parsed from Concord's `[Waiting on lock owned by Thread 0x..]` annotation; live-verified on LockJam). Still not decoded: *which object* a blocked thread is waiting to enter (ClrMD can't read the `Monitor.Enter` argument — cross-reference the two tools to close the exact edge), and non-monitor primitives (`SemaphoreSlim`/`ReaderWriterLockSlim`) beyond their managed fields.
- **Async call stacks: logical chain now reconstructed.** `vs_async_stacks` (ClrMD) rebuilds the *logical* `InnerAsync ← ComputeAsync ← RunAsync` chain from the heap's state-machine boxes, each with its await-point `state` — the chain the *physical* stack (`MoveNext`, async-builder internals, `ThreadPool…`) hides (live-verified on AsyncTrace). Residual: a suspended async caller's hoisted locals still aren't reachable by source name, and which awaitable each frame is parked on isn't decoded (only the await-index `state`).
